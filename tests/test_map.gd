...
func _run() -> void:
	...
	await process_frame
	check(game.map.markers().size() == 4, "all three NPCs and local player mapped offline")
	...
	check(canvas.bounds.has_point(Vector2(95, 20)), "map includes explorable jungle")
	...
	check(game.map.markers().size() == 5, "party has one local marker and friend")
	...
	friend.queue_free()
	check(game.map.markers().size() == 4, "departed friend removed")
	key(KEY_M)
