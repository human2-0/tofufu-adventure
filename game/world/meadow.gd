...
func _river_and_bridge() -> void:
	...
	for side in [-1.0, 1.0]:
		...
		MeadowGeometry.box(self, Vector3(11, 0.85, 4 + side * 1.4), Vector3(8.3, 0.12, 0.12), Color("d7b383"))
	for i in 70:
		var z := _rng.randf_range(-40, 40)
		if absf(z - 4) < 3.0:
			continue
		var x := terrain.river_x(z) + (-1.0 if i % 2 == 0 else 1.0) * _rng.randf_range(2.9, 3.5)
		MeadowGeometry.rock(self, ground_point(x, z), Vector3(0.4, 0.25, 0.3), Color("9eac89"))
...
func _farmstead() -> void:
	...
	for x in [-2.6, -1.4, 1.5, 2.7]:
		MeadowGeometry.box(bank, Vector3(x, 0.35, 3.2), Vector3(0.8, 0.7, 0.7), Color("bb915b"))
	...
	MeadowGeometry.rock(self, ground_point(-28, -23, 4.2), Vector3(1.65, 0.75, 1.65), Color("80a193"))
	...
	MeadowGeometry.box(self, at + Vector3.UP, Vector3(0.13, 2, 0.13), Color("8e7351"))
	MeadowGeometry.box(self, at + Vector3.UP * 1.4, Vector3(1.5, 0.45, 0.25), Color("bf8a65"))
	MeadowGeometry.rock(self, at + Vector3.UP * 1.9, Vector3.ONE * 0.3, Color("f3d694"))
	MeadowGeometry.rock(self, at + Vector3.UP * 2.12, Vector3(0.48, 0.1, 0.4), Color("957552"))
...
func _village() -> void:
	...
	for side in [-1.0, 1.0]:
		MeadowGeometry.box(self, well + Vector3(side * 0.9, 1.25, 0), Vector3(0.15, 2.5, 0.15), Color("8e7558"))
	MeadowGeometry.box(self, well + Vector3.UP * 2.5, Vector3(2.5, 0.2, 1.9), Color("b98773"))
	...
	MeadowGeometry.box(self, anvil + Vector3.UP * 0.95, Vector3(1.35, 0.35, 0.6), Color("66747c"))
